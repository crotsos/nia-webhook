...
def get_intent(username, origin, destination, target, middleboxes, qos, start, end, allow, block):
    ...
    if target:
        intent = intent + ' for client("' + target + '")'
...
def get_entities(username, origin, destination, target, middleboxes, qos, start, end, allow, block):
    entities = username
    if origin:
...
def read():
    ...
    for line in lines:
        if line and not line.startswith('#'):
            ...
            input_words.append(text_to_word_sequence(input_text, filters='!"#$&()*+,-./;<=>?@[\\]^_`{|}~\t\n'))
            output_words.append(text_to_word_sequence(output_text, filters='!#$&*+-./;<=>?@[\\]^_`{|}~\t\n'))
...
def read_split():
    ...
    for line in fit_lines:
        if line and not line.startswith('#'):
            input_text, output_text = line.split('>')
            fit_input_words.append(text_to_word_sequence(input_text, filters='!"#$%&()*+,-./;<=>?@[\\]^_`{|}~\t\n'))
            fit_output_words.append(text_to_word_sequence(output_text, filters='!"#$%&()*+,-./;<=>?@[\\]^_`{|}~\t\n'))
