...
data <- read.csv("/home/asjacobs/workspace/nia-webhook/res/dataset_results.csv")
data$id <- as.character(data$id)
data$num_id <- as.numeric(data$id)
data[with(data, order(num_id)), ]

datasets <- c(100, 500, 1000, 2000, 5000)
times <- c(5.83333, 17.5, 36.16667, 67.66667, 166.833336)

time <- data.frame(datasets, times)

feedback_100 <- read.csv("/home/asjacobs/workspace/nia-webhook/res/100_feedback_results.csv")
feedback_100$dataset <- 100
feedback_500 <- read.csv("/home/asjacobs/workspace/nia-webhook/res/500_feedback_results.csv")
feedback_500$dataset <- 500
feedback_1000 <- read.csv("/home/asjacobs/workspace/nia-webhook/res/5.833331000_feedback_results.csv")
feedback_1000$dataset <- 1000
feedback_2000 <- read.csv("/home/asjacobs/workspace/nia-webhook/res/2000_feedback_results.csv")
feedback_2000$dataset <- 2000
feedback_5000 <- read.csv("/home/asjacobs/workspace/nia-webhook/res/5000_feedback_results.csv")
...
m <- ggplot(data, aes(x=lid, y=mean_rsquared, group = 1)) 
m + 
  geom_point(stat="identity") +
  geom_line(stat="identity") +
  ...
  xlab("Dataset Size") + ylab("R-squared") +
  theme_bw() +  
  theme(axis.line.x = element_line(color = 'black'),
        axis.line.y = element_line(color = 'black'),
        legend.justification=c(0,1), 
